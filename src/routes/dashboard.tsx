...
import { getCurrentAvatarRequest } from '@/media/media-api'
...
function DashboardPage() {
  const { tab, project_id, workspace_tab, chat_channel, chat_message_id } = Route.useSearch()
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        ...
        // El estado local se limpia siempre, incluso si el backend ya no conoce la sesion.
      }
    },
    onSettled: () => {
      clearSession()
      ...
      <div className="flex items-center justify-center min-h-screen px-4">
        <div className="w-full max-w-sm space-y-4">
          <Alert variant="destructive">
            ...
            <AlertDescription>
              La sesion no trajo informacion de usuario. Reintenta o vuelve a iniciar sesion.
            </AlertDescription>
          </Alert>
          <Button variant="outline" className="w-full" onClick={() => dashboardQuery.refetch()}>
            Reintentar
    ...
    >
      ...
      {activeTab === 'collab' && (
        <CollabPanel
          ...
          initialWorkspaceTab={workspace_tab}
